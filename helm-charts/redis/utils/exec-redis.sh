...
POD_NAMES=`kubectl get pods -n ${NAMESPACE} --no-headers -o wide | grep "^redis-" | grep -v "^redis-sentinel-" | awk '{ print $1":"$7 }'`
...
./pod-exec-template.sh "${NAMESPACE}" "${ANSWER_REQUESTION_RESPONSE}" "exec" "-it -- sh"
